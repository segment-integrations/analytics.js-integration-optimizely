...
Optimizely.prototype.initialize = function() {
  ...
  initOptimizelyIntegration({
    referrerOverride: setEffectiveReferrer,
    sendExperimentData: this.sendClassicDataToSegment,
    sendCampaignData: this.sendNewDataToSegment
  });
...
Optimizely.prototype.sendClassicDataToSegment(experimentState) {
  ...
  if (this.settings.listen) {
    ...
    if (variations.length > 1) {
      // Note: Legacy code was improperly sending the name of just one variation even when there were multiple
      // So just like variationIds, we will concatenate the names for multivariate experiments
      // This isn't necessarily a "breaking" change since this is augmentative data
      ...
      // TODO: what is sent under props.sectionName? I do not see this data point in the snippet nor inside `window.optimizely.data`
      // Unsure if this was ever being sent??
    }
  ...
  if (this.settings.variations) {
    ...
    var traits = {};
    traits['Experiment: ' + experiment.name] = variationNames.join(); // in case it is multivariate
...
Optimizely.prototype.sendNewDataToSegment(campaignState) {
...
function setEffectiveReferrer(referrer) {
