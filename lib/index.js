...
Optimizely.prototype.sendClassicDataToSegment = function(experimentState) {
  ...
  if (this.options.listen) {
    ...
    if (sections) {
      ...
      // So we have to do these terrible for loops as a workaround.
      var sectionNames = [];
      var sectionIds = [];
      var sectionMap = {};
      var activeSections = {};

      // We need to reformat the data structure to reduce complexity from 4x to 2x
      ...
      // Experiments can display variations from multiple sections so we are checking which
      // sections were actually used
      for (var j = 0; j < variationIds.length; j++) {
        var activeSection = sectionMap[variationIds[j]];
        if (activeSection && !activeSections[activeSection.id]) {
