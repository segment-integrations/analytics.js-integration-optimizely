 ...
 * @param {Object} experimentState.sections[sectionId].name: the name of section
 * @param {Object} experimentState.sections[sectionId].variation_ids: the IDs of the variations in the section
 *
 */
...
Optimizely.prototype.sendClassicDataToSegment = function(experimentState) {
  ...
  if (this.options.listen) {
    var props = {
      experimentId: experiment.id, // to be consistent with other ids
    ...
    if (variations.length > 1) {
      // All the variations inside `section[sectionId].variation_ids` might not be the combination of variations that the
      // user might be _actually_ seeing so this is the accurate way to track which variation(s) actually displayed on the page
      props.variationName = variationNames.join(', '); // eg. 'Variation #1, Variation #2, Var 3709'
      ...
      if (sections) {
        ...
        // We have to do these terrible for loops to workaround how Optimizely exposes their section data
        var sectionNames = [];
        var sectionIds = [];
        var sectionMap = {};
        var activeSections = {};

        // We need to do this to reduce complexity from 4x to 2x
        for (var sectionId in sections) {
          if (!sections.hasOwnProperty(sectionId)) continue;
          var section = sections[sectionId];
          var sectionVariationIds = section.variation_ids;
          for (var i = 0; i < sectionVariationIds.length; i++) {
            sectionMap[sectionVariationIds[i]] = {
              id: sectionId,
              name: section.name
            };
          }
        ...
        for (var j = 0; j < variationIds.length; j++) {
          var activeSection = sectionMap[variationIds[j]];
          if (section && !activeSections[activeSection.id]) {
            sectionNames.push(activeSection.name);
            sectionIds.push(activeSection.id);
            activeSections[activeSection.id] = true; // For deduping purposes
          }
        }

        props.sectionName = sectionNames.join(', ');
        props.sectionId = sectionIds.join();
      }
    }
 ...
 * @param {Object} campaignState.experiment.referrer: the effective referrer of the experiment (only defined for redirect)
...
Optimizely.prototype.sendNewDataToSegment = function(campaignState) {
  ...
  if (this.options.listen) {
    ...
    // For redirect experiments this is available
    if (experiment.referrer) props.referrer = experiment.referrer;
