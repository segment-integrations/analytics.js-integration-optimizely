...
Optimizely.prototype.roots = function() {
  ...
  if (redirectExperiment) {
    ...
    if (data.state.activeExperiments.indexOf(id) === -1) {
      activeExperiments.push({
        ...
        variationName: data.variations[variationId].name,
        experimentName: data.experiments[id].name,
        experimentId: id,
        referrer: referrer
      });
    }
  ...
  window.console.log("roots");
  window.console.log(activeExperiments);
...
function getExperiments(options) {
  window.console.log("getExperiments");
  window.console.log(options.activeExperimentIds);
